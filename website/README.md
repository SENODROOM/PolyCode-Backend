# 🐍 Java Docs Hub — MERN Stack

A full-stack MERN documentation website that **automatically reads your Java folder** (or exported `.txt` file), parses every `.md` and `.py` file, converts them to beautiful HTML, and displays them with live syntax-highlighted code examples.
...
| Feature | Details |
|---|---|
| 🔄 **Auto-Ingestion** | Reads your Java folder on startup — no manual imports |
| 🐍 **Java Syntax Highlighting** | Every `.py` file rendered with full highlighting |
...
| 👀 **Live File Watching** | Changes to your Java folder auto-update the DB |
...
# Edit backend/.env if needed (default: mongodb://127.0.0.1:27017/java_docs)
```

### 4. Place your Java documentation

**Option A — Java folder** *(recommended)*
Place (or symlink) your Java folder next to `java-docs-mern/`:

```
parent-folder/
├── Java/          ← your existing Java folder
└── java-docs-mern/
```

**Option B — Exported .txt file**
Copy your `Java.txt` export into the data folder:

```bash
mkdir -p backend/../data
cp /path/to/Java.txt data/Java.txt
```
...
├── data/                 ← put Java.txt here (Option B)
│
├── backend/
...
│   └── utils/
│       └── fileParser.js ← Markdown/Java parser + HTML converter
│
└── frontend/
    └── src/
        ├── App.js
...
1. **`../Java/` folder** — walks the whole tree, parses every `.md` and `.py` file
2. **`./data/Java.txt`** — parses the exported monolithic dump file
...
*Built for the Java Development Hub — March 2026*
