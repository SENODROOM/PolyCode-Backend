...
export default function DocumentPage() {
  ...
  const isJava = doc.fileType === 'java' || doc.fileType === 'py';

  return (
    <article className={`fade-up doc-container ${isJava ? 'code-mode' : 'reading-mode'}`}>
      {/* Breadcrumb */}
      <nav className="breadcrumb">
        ...
        <span className="sep">/</span>
        <span className="active-path">{doc.title}{isJava ? '.py' : '.md'}</span>
      </nav>

      {/* Header - Only for Non-Java/Documentation files */}
      {!isJava && (
