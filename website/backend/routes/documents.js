...
// Path to your Java folders
const JAVA_BASE_PATH = path.join(__dirname, '../../../data');
...
async function checkDataStructure() {
  try {
    ...
    if (await fs.access(JAVA_BASE_PATH).then(() => true).catch(() => false)) {
      console.log('✓ Data folder found at:', JAVA_BASE_PATH);
    } else {
      console.log('✗ Data folder not found at:', JAVA_BASE_PATH);
    }
  } catch (error) {
...
async function scanDirectory(dirPath, basePath = JAVA_BASE_PATH, maxDepth = 3, currentDepth = 0) {
  const items = [];
...
router.get('/', async (req, res) => {
  try {
    ...
    // Scan data directory
    if (await fs.access(JAVA_BASE_PATH).then(() => true).catch(() => false)) {
      const dataDocs = await scanDirectory(JAVA_BASE_PATH);
    ...
    if (fileType && fileType !== 'all') {
      ...
      if (fileType === 'py') targetType = 'java';
...
router.get('/*path', async (req, res) => {
  try {
    ...
    let fullPath = path.join(JAVA_BASE_PATH, requestedPath);
    
    // Security check - ensure the path is within the data directory
    const resolvedPath = path.resolve(fullPath);
    const resolvedBase = path.resolve(JAVA_BASE_PATH);
    ...
    if (fileInfo.fileType === 'markdown') {
      let allDocuments = [];
      if (await fs.access(JAVA_BASE_PATH).then(() => true).catch(() => false)) {
        allDocuments = await scanDirectory(JAVA_BASE_PATH);
      }
